...
*   皮肤电数据持久化线程类
* */
public class GSRThread implements Runnable{
    private static final Logger logger = LoggerFactory.getLogger(GSRThread.class);

    @Override
    public void run() {
        logger.info("GSR thread started");
        gsrDataStore();
    }
    ...
    public GSRThread(String experiment_id, RedisTemplate redisTemplate, CnosdbUtil cnosdbUtil,int start,int end) {
    ...
    private void gsrDataStore(){
        logger.info("GSR store started");
        // 获取所有的实验数据
        List<GSR> datas = redisTemplate.opsForList().range(RedisKeyConfig.EXPERIMENT_DATA_GSR_KEY,start,end);
            ...
            try{
                ...
                for (GSR temp : datas) {
                    ...
                    try {
                        ...
                        StringBuilder insertSQL = new StringBuilder(CnosDBProperties.GSR_INSERT + " values(");
                        // 构建SQL语句
...
//                cnosdbUtil.closeResources();
            ...
            logger.info("No GSR data to store.");
        }
